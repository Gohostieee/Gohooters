		...
		if(view | view===0){
			if(!view){
				...
				for(let x = 0; x < props.options.length;x++){
 					uwu.push(
						<div class="flex flex-row options justify-center mb-10 ">

			 				<p onClick={()=>{setViewState(x+1)}}class="text-white text-3xl QuickSand mice option careerOpt opt-1">{props.options[x][0]}</p>
						</div>
					)
				...
				<div class="m-auto border-x-2 h-[70%] mt-12 w-[80%] flex-col flex justify-evenly mt-12 mb-12 overflow-y-visible">
					{uwu}
				</div>
					)};
			return (
				<div class="m-auto  h-[85%]  lg:w-[80%] sm:w-[100%] flex-col flex mt-3 overflow-y-visible">
					...
					<div class="flex flex-row justify-center mb-2 ">
		      		...
		      		<Link to={props.options[parseInt(view)-1][2][1]} ><button class="btn mt-8 lg:mt-0 option btn-outline btn-primary glitch text-xl font-thin layers " data-text="Enter">{props.options[parseInt(view)-1][2][0]}</button></Link>
