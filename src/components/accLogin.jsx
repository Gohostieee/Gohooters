...
export default class AccLogin extends Component{
    ...
    createInput(x){
        return (
            <div className="form-control w-[80%]  max-w-xs text-white m-auto">
                <label className="label">
                    <span className="label-text text-white">{x[0]}</span>
                </label>
                <input type="text" id = {x[1]}className="input mb-8 input-bordered w-full max-w-xs bg-transparent border-white rounded-l " />
            </div>
        )
    }
    createError(x){
        ...
        return (
            ...
            <>
                <span className="text-red-500 text-l QuickSand tracking-[3px] text-thin underline underline-offset-4">{x}</span>
...
//FUTURE ME NOTE!! REVAMP REVAMP TOO MANY SWITCHES,TOO ANNOYING TO READ/DEBUG, CODE COMMENTED OUT
    ...
    render(){
        ...
        return(
            <>
                <div className="border-x  flex flex-col justify-c w-[30%] m-auto sm:mt-12 mt-8 min-w-[350px]">

                    {this.state.inputs[this.state.request]}
                    {this.state.errors}
                </div>

                <button onClick={() =>{this.handleInput('login',$("#password").val(),$("#username").val())}} className="btn option btn-outline w-[330px] btn-primary glitch text-base sm:text-xl font-thin layers mt-8 " id="loginButton" data-text="WELCOME BACK">LOGIN</button>
                <br/>
                <button onClick={() =>{this.handleInput('signup',$("#password").val(),$("#username").val(),$("#email").val())}} className="btn option btn-outline w-[330px] btn-primary glitch text-base sm:text-xl font-thin layers sm:mt-16 mt-4" id="signupButton" data-text="DO YOU WISH TO PROCEED.">CREATE ACCOUNT</button>
                <br/>
